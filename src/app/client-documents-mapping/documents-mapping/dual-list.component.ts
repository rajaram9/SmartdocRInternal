...
@Component({
  selector: 'si-dual-list',
  styleUrls: ['record-picker.css', 'documents-mapping.component.css'],
  templateUrl: 'documents-mapping.component.html'
})
...
export class DualListComponent implements DoCheck, OnChanges {
  ...
  sorter = (a: any, b: any) => { return (a._name < b._name) ? -1 : ((a._name > b._name) ? 1 : 0); };


  constructor(private differs: IterableDiffers, private cdr: ChangeDetectorRef
    , private clientdocumentsmappingService: ClientDocumentsMappingService
  ) {
  }
  ...
  ngDoCheck() {
    const sourceChanges = this.sourceDiffer.diff(this.source);
    if (sourceChanges) {
    ...
    const destChanges = this.destinationDiffer.diff(this.destination);
    if (destChanges) {
  ...
  drop(event: DragEvent, list: BasicList) {
    ...
    const mv = list.list.filter((e: any) => e[this.key] == id);
  ...
  trueUp() {
    ...
    // Push added items.
    for (let i = 0; i < this.confirmed.list.length; i += 1) {
  ...
  moveItem(source: BasicList, target: BasicList, item: any = null) {
    ...
    for (; i < len; i += 1) {
      ...
      if (mv.length === 1) {
        ...
        // Make unavailable.
        const idx = source.list.indexOf(mv[0]);
        if (idx !== -1) {
  ...
  shiftClick(event: MouseEvent, index: number, source: BasicList, item: any) {
    if (event.shiftKey && source.last && !Object.is(item, source.last)) {
      const idx = source.list.indexOf(source.last);
      if (index > idx) {
  ...
  save() {
    const saveObj = {
      ...
      DocumentIDs: this.destination.map(elem => elem.key).join(',')
    };

    this.clientdocumentsmappingService.saveClientDocumentMappings(saveObj).subscribe(
      data => { },
      error => { }
    );
  }
